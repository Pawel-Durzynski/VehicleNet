...
public sealed class VehicleBodyEngineService : IVehicleBodyEngineService
{
    ...
    public VehicleBodyEngineSearchResult Search(VehicleBodyEngineSearchCriteria criteria, CancellationToken cancellationToken = default)
    {
        ...
        IEnumerable<VehicleBodyEngine> query = _vehicles;
        ...
        if (criteria.VehicleBodyEngineId.HasValue)
        {
            query = query.Where(v => v.VehicleBodyEngineId == criteria.VehicleBodyEngineId.Value);
        }
        ...
        return new VehicleBodyEngineSearchResult(filtered.Count, filtered);
    }
